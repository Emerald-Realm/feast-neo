...
export default function Home({ posts }: Props) { 
  ...
  return (
    ...
    border-y border-block py-10 lg:py-0">
       <div className="px-10 space-y-5">
         <h1 className="text-6xl max-w-xl font-serif"><span 
         className='underline decoration-black decoration-4'>Medium</span> is a place to write, read and connect</h1>
       ...
       <Link key={post._id} href={`/post/$post.slug.current`}>
          ...
          overflow-hidden'>
            ...
            <div className='flex justify-between p-5  bg-white'>
              <div>
                <p>{post.title}</p>
                <p>{post.description} by {post.author.name}
                </p>
              </div>
              <img 
